 ...
 * {@code endOfInput()}).
 *
 * <p>GHSA-r7wm-3cxj-wff9: incomplete fix for GHSA-72hv-8253-57qq — a stream of
 * digit-only chunks could grow the async parser's buffer past
 * {@code maxNumberLength} because the streaming {@code NOT_AVAILABLE} yield points
 * did not validate the accumulated integer length.
 */
public class AsyncNumberLengthConsistencyTest
    extends BaseTest
{
    // Default StreamReadConstraints max number length used by newStreamFactory().
    ...
    public void testIntegerPathStreamingChunksRejectsBeyondMaxNumberLength() throws Exception {
        ...
        try {
            ...
            try {
                ...
                assertTrue("Unexpected message: " + msg,
                        msg.contains("exceeds the maximum allowed"));
